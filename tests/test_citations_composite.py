...
back-compat, and ``excerpt_tokens`` carries every verified member literal (each a
verbatim element at ``field_path``). A single-value bucket is unchanged:
``excerpt_tokens is None``.
"""
...
def test_composite_bucket_carries_every_verified_member_token() -> None:
    """PHASE1|PHASE2 composite → excerpt=="PHASE1", excerpt_tokens==["PHASE1","PHASE2"];
    ...
    for citation, record in zip(citations, records, strict=True):
        ...
        assert citation.excerpt == "PHASE1"  # first token, for display
        assert citation.excerpt_tokens == ["PHASE1", "PHASE2"]  # in token order
        ...
        for token in citation.excerpt_tokens:
...
def test_single_value_bucket_via_member_tokens_is_back_compatible() -> None:
    """member_tokens with length 1 → single-value behavior: excerpt=="PHASE3",
    excerpt_tokens is None."""
    ...
    for citation in citations:
        assert citation.excerpt == "PHASE3"
        assert citation.excerpt_tokens is None  # single-value → no composite tokens


def test_member_tokens_none_is_unchanged_single_value_behavior() -> None:
    """member_tokens=None (the default) → excerpt=="PHASE3", excerpt_tokens is None."""
    records = [_record(f"NCT{i:08d}", phases=["PHASE3"]) for i in range(3)]
    ...
    for citation in citations:
        assert citation.excerpt == "PHASE3"
        assert citation.excerpt_tokens is None
...
def test_composite_honesty_absent_member_token_is_excluded() -> None:
    ...
    excerpt_tokens (honesty check) — no synthesized/unverifiable literal."""
    ...
    assert by_nct["NCT00000001"].excerpt == "PHASE1"
    assert by_nct["NCT00000002"].excerpt == "PHASE1"
    # excerpt_tokens is honest: only the tokens actually present in each record:
    assert by_nct["NCT00000001"].excerpt_tokens == ["PHASE1", "PHASE2"]
    assert by_nct["NCT00000002"].excerpt_tokens == ["PHASE1"]  # PHASE2 excluded
...
def test_composite_reports_exact_contributing_count_and_truncates_at_k() -> None:
    ...
    truncated; each citation still carries the composite excerpt_tokens."""
    records = [
    ...
    assert all(c.excerpt_tokens == ["PHASE1", "PHASE2"] for c in citations)
